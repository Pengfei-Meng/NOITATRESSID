...
The first test problem to verify RSNK's globalization performance is an analytical inequality constrained Sphere problem: 
\begin{equation*}
\begin{aligned}
\underset{x, y, z} {\text{min}}  & \quad \quad x + y + z \\
   {\text{subject to}}  & \quad \quad x^2 + y^2 + z^2 <= 3 \\
\end{aligned}
\end{equation*}
...
\subsection{Results}
To prove that the algorithm can bypass the local maximum, 100 random starting points around the local maximum $(1,1,1)^T$ are generated using Gaussian random distribution, with a mean at 1, and a standard deviation 0.1. Figure~\ref{fig:sphere_100} shows the sphere, with the starting points in blue circles, and the solutions in red stars. 
As can be seen, even when the 100 starting points are all around the local maximum, they all converge to the minimum point as indicated by the red stars. The thick red star are actually 100 points stick together. 

 \begin{figure}[H]
  \centering
  \includegraphics[clip,trim=20 40 20 70, width=0.8\columnwidth]{./figs/chap2/100_111.pdf}%  
...
Figure~\ref{fig:sphere_1000} shows the scatter plot of 1000 starting points randomly generated using the Standard 
Normal Distribution method in semi-transparent blue circles. They all converge to the solid red star point, which is actually 1000 solutions points overlaid on top of each other. Figure~\ref{fig:sphere_50} is a less crowded symbolic
plot with 50 starting points. 

The test shows that the globalized RSNK method possesses fairly robust globalization ability that bypasses local maximum points  
