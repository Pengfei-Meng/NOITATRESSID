...
\begin{equation*}
  ...
  \end{bmatrix}^T, 
\end{equation*}
for which $q_0 = \begin{bmatrix} x_0^T & g(x_0)^T & 0^T & 0^T \end{bmatrix}$.
...
stages of convergence.  Furthermore, the term $\mu(s - g(x_0)$ generates a path
that remains feasible with respect to the inequality constraints.  Finally, the
...
is equal to zero, and the solution of the primal-dual system is recovered.
...
\begin{description}
  ...
  \item[Predictor:] During the predictor phase we first need to compute an
    ...
    decreasing $\mu$ as the path moves from $\mu=1$ to $\mu=0$.
  ...
  \item[Corrector:] In the corrector phase, we fix the homotopy parameter
    ...
    provided in Section~\ref{sec:matfree}.
...
\begin{itemize}
  \item During the predictor phase, the maximum allowable step size is bounded
    using the fraction-to-the-boundary rule~\cite{Nocedal2006NO}.
    \begin{align*}
      \alpha_{\max}^{\textsf{prim}} &= \max\left\{ \alpha \in (0,1] \;|\; s + \alpha s' \geq (1-\tau) s \right\}, \\
        \alpha_{\max}^{\textsf{dual}} &= \max\left\{ \alpha \in (0,1] \;|\; \lambda_g + \alpha \lambda_g' \geq (1-\tau) \lambda_g \right\}.
    \end{align*}
    As is common, we use the first bound for both the design and slack
    variables, and we use the second bound for the multipliers.
    
  \item After solving for the correction $\Delta q_{k+1} = q_{k+1} -
    \hat{q}_{k+1}$, the slacks and inequality multipliers are clipped to ensure
    they have the correct signs:
    \begin{equation*}
      s_{k+1} = \max(\hat{s}_{k+1} + \Delta s_{k+1}, 0)
      \qquad\text{and}\qquad      
      \lambda_{g,k+1} = \min(\hat{\lambda}_{g,k+1} + \Delta \lambda_{g,k+1}, 0),
    \end{equation*}
    where the $\max$ and $\min$ functions in the above expressions are to be
    interpreted elementwise.
\end{itemize}

\begin{remark}
  We assume that a solution to \eqref{eq:opt00x} satisfies strict
  complementarity, that is, either $s_i = 0$ or $\lambda_{g,i}=0$, but not both
\end{remark}
...
explanation.  The solution of these linear systems is detailed in the next
section.
...
\Parameter{$K_{\max}$, $J_{\max}$, $\epsilon_F$, $\epsilon_H$, $\alpha_0$, $\delta_{\text{targ}}$, $\phi_{\text{targ}}$, $\Delta \mu_{\max}$}
\Input{$x_0$}
\Output{$q^{*} = \begin{bmatrix} x^{*T} & s^{*T} & \lambda_h^{*T} & \lambda_g^{*T}   \end{bmatrix}^T$, the solution of \eqref{eq:opt00x}}
\BlankLine
Set $q_0 = \begin{bmatrix} x_0^T & g(x_0)^T & 0^T  & 0^T \end{bmatrix}$,
$\delta_{0} \leftarrow \delta_{\text{targ}}$ and
$\phi_{0} \leftarrow \phi_{\text{targ}}$\\
\For{$k = 0,1,2,\ldots,K_{\max}$}{
  \lIf{$\| F(q_{k}) \| \leq \epsilon_{F} \|F(q_{0})\|$,  $s_{i} \geq 0$,
    and $\lambda_{gi} \leq 0$}{\Return $q_{k}$}
  \ShowLn
  inexactly solve $\left(\nabla_q H\right)_{k} q_{k}' = - \nabla_\mu H_{k}$
  for $q_{k}'$ \label{line:tang} \\
  $\zeta_{k} = \max\left( \sqrt{\delta_k/\delta_{\text{targ}}}, \phi_k / \phi_{\text{targ}} \right)$\\
  compute $\alpha_{k} = \min\left( \sqrt{\|q_{k}'\|^2 + 1}\Delta \mu_{\max}, \alpha_{\max}, \alpha_{k-1}/\zeta_{k} \right)$\\
  apply \eqref{eq:pred} (the predictor) to get $\hat{q}_{k+1}$ and $\mu_{k+1}$\\
  ...
  clip if necessary: $s_{k+1} \leftarrow \max(s_{k+1},0)$ and
  $\lambda_{g, k+1} \leftarrow \min(\lambda_{g, k+1},0)$ \\
  update $\delta_k$ and $\phi_{k}$ according to \eqref{eq:delta_k} and
  \eqref{eq:phi_k} \\
}
\caption{Inexact-Newton predictor-corrector algorithm for constrained
