...
\begin{figure}[tbp]
  \centering
   \subfloat[small \label{fig:fs1}]{
   \includegraphics[clip,width=0.5\textwidth]{./figs/chap6_fstopo/1.eps} } 
   \subfloat[small \label{fig:fs3}]{
   \includegraphics[clip,width=0.5\textwidth]{./figs/chap6_fstopo/3.eps} } \\
  \subfloat[medium \label{fig:25}]{
   \includegraphics[clip,width=0.5\textwidth]{./figs/chap6_fstopo/25.eps} }
    \subfloat[medium \label{fig:27}]{
   \includegraphics[clip,width=0.5\textwidth]{./figs/chap6_fstopo/27.eps} }\\
    \subfloat[large \label{fig:29}]{
   \includegraphics[clip,width=0.5\textwidth]{./figs/chap6_fstopo/29.eps} }
    \subfloat[large \label{fig:32}]{
   \includegraphics[clip,width=0.5\textwidth]{./figs/chap6_fstopo/32.eps} }
...
Last but not least, it is worth noting that, the small amount of regularization $\epsilon_{\mu}=10^{-6}$  
...
\begin{table}[tbp]
  \begin{center}
    ...
    \label{tab:aero_kona}}
  \begin{tabular}{ l  c | c c c }
%       &   \multicolumn{3}{ c }{192}   &   \multicolumn{3}{ c  }{480} &   \multicolumn{3}{ c }{768}   \\
%       \hline
         & Size  & $C_D \times 10^4$ & $C_L$ & $C_{My}$   \\         
    \hline
    % \rule{0ex}{3ex}%
 \multicolumn{2}{ l |}{Initial}  &    108.5017 & 0.489247 & -0.195020    \\ 
  \multirow{ 3}{*}{Optimal} & 192 &  96.1716 & 0.499805 & -0.170138  \\
...
%       \hline
    ...
    \hline
    % \rule{0ex}{3ex}%
 \multicolumn{2}{ l |}{Initial}  &    108.5025 & 0.489247 & -0.195021    \\ 
  \multirow{ 3}{*}{Optimal} & 192 &  97.9155 & 0.499997 & -0.169998  \\
...
%In addition, in the convergence plots Figure~\ref{fig:kona_192} to \ref{fig:sn_768}, the starting aerodynamic coefficients in SNOPT are slightly smaller than that in Kona, that was found to be because the angle of attack, which is part of the design variables, was set to $2.2^{\circ}$ in SNOPT and $2.5^{\circ}$ in Kona. The geometric design variables are the same between the two. The difference in the initial angle of attack was not intentional and would not change the convergence histories in a big way, except that the aerodynamic coefficients would start the same from 
%Figure~\ref{fig:kona_192} to \ref{fig:sn_768}. 
