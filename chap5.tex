...
and is normalized by the norm of the current iterative point:
\begin{equation*}
\text{SNOPT feasibility} = \underset{i}{\text{max}}  \  c_i / \lVert x \rVert, \quad \text{where} \quad c= \left[ ( c(x) - cl)^T, (cu -c(x) )^T \right]   
\end{equation*}        
where $x$ is the current iterative point, and $c(x)$ is defined in Appendix~\ref{sec:cuternm}. 
...
\end{equation*}
where  $\pi$ is the Lagrangian multiplier vector; 
$\text{Comp}_j$ measures the complementarity slackness for $j$th variable and is defined by:
\begin{equation*}
...
\item The parameters \textbf{$\alpha_0$},  $\delta_{\text{targ}}$, $\phi^{\circ}_{\text{targ}}$,  $n_{\mat{\Sigma}}$ play important roles in the convergence speed of the new algorithm. In some cases the parameters have to be changed manually to get convergence. In contrast, the parameters for SNOPT remained unchanged for all the test problems. 
...
\item If the null space of the inequality constraint Jacobian includes a constant   a vector of ones, then the preconditioner will fail. This is because Kona will start the Lanczos algorithm (used to build the preconditioner) with a vector of ones.
 For the problems whose inequality constraint Jacobian have certain structure, \eg $[-1, -3, 3, 1]$ on each row like the LISWET problems, the matrix vector product will be zero, causing the Lanczos method to crash. 
\end{enumerate} 
...
Note that in Kona, the $C_L$, and $C_{My}$ do not conform to the inequality constraints very quickly, and they 
