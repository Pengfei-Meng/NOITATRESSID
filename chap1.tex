...
In such applications, an optimization library is coupled with PDE solvers. The optimizer will ask for the function values and gradients of the objective and constraints, which entail the solutions of the PDE solvers and gradient solvers. Figure~\ref{fig:outflow} illustrates the schematic diagram of the optimization process.   
 ...
 \begin{figure}[H]
  \centering
  \includegraphics[clip, width=1.0\columnwidth]{./figs/chap1_intro/1_optFlow.png}%  
...
Using $\mathcal{L}$, the first-order necessary, or the \textit{Karush-Kuhn-Tucker} (KKT) optimality conditions for \eqref{eq:gen1} can be expressed as
\begin{equation}\label{eq:kktcond}
\begin{aligned}
\nabla_x \mathcal{L} &= \nabla_x f + \lambda_h^T \nabla_x h + \lambda_g^T \nabla_x g + \psi^T \nabla_x\mathcal{R} = 0, \\
\nabla_u \mathcal{L} &= \nabla_u f + \lambda_h^T \nabla_u h + \lambda_g^T \nabla_u g + \psi^T \nabla_u\mathcal{R} = 0, \\
\nabla_{\psi} \mathcal{L} &= \mathcal{R} = 0, \\
...
treat the states $u$ and the adjoints $\psi$ as implicit functions of the design variables through $\mathcal{R}(x,u(x)) = 0$, and $\nabla_u \mathcal{L} = 0$. Consequently, the reduced KKT conditions can be formulated as follows:
...
\end{gathered}
\end{equation}
Where the unknown are $x^T, s^T, \lambda_h^T, \lambda_g^T$
...
The challenges include globalization, which refers to the ability to bypass local minimizers, and nonconvexity handling, which involves by passing maximizers. The first challenge is addressed in Chapter~\ref{chap:homotopy} and the second in Chapter~\ref{chap:linsys}. 
